...
describe('Login flow', () => {
  ...
  it('should simulate login with mocked MSAL', () => {
    console.log('username', window.msalInstance.getAllAccounts());

    // Visit the root of the app
    cy.visit('/');

    // cy.window().then((win) => {
    //   console.log('test window.. ', win);
    //
    //   expect(win.msalInstance).to.exist;
    //   const accounts = win.msalInstance.getAllAccounts();
    //   expect(accounts).to.have.length.greaterThan(0);
    //   expect(accounts[0].username).to.equal('testuser@domain.com');
    // });
  });
});
