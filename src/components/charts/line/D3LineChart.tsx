...
type BulletProps = {
  ...
  values: number[];
  /** Optional title shown above the bullet. */
  title?: string;
...
const BulletMinMeanMax: React.FC<BulletProps> = ({
  values,
  ...
  const x = (v: number) =>
    ((v - stats.min) / (stats.max - stats.min || 1)) * innerW;
      ...
      >
        ...
        <g transform={`translate(${innerMargin.left},${innerMargin.top})`}>
          ...
          <rect
            ...
            ry={10}
            fill="url(#band-matte)"
            stroke={TABLEAU10.teal}
            strokeWidth={1}
            filter="url(#emboss)"
          />

          {/* Mean marker (Tableau10 blue) */}
          <line
            x1={x(stats.mean)}
            x2={x(stats.mean)}
            y1={barY - 7}
            y2={barY + barH + 7}
            stroke={TABLEAU10.blue}
            strokeWidth={8}
            filter="url(#emboss)"
          />
          <circle
...
export default BulletMinMeanMax;
