...
const mapping = (data: any, dataS: StandardData, dataContinuum: ContinuumData) => {
...
function getContinuumData(
  ...
  if (mocked) {
    return Promise.resolve(CONTINUUM_DATA_MOCKED);
  } else {
    ...
    if (showAdvanced && !isLow(telescope)) {
      properties += addAdvancedData(standardData.advancedData);
    }
