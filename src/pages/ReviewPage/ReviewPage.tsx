...
export default function ReviewPage() {
  ...
  const colNewField = {
    field: 'newField',
    headerName: 'New Field', //t('status.label')
    width: 120,
    renderCell: (e: { row: any }) =>  e.row.newField? e.row.newField: 'N/A' //TODO: replace NA with default value or delete NA after field is added   
  };

  const colRank = {
    field: 'rank',
    headerName: t('rank.label'), // t('status.label')
    width: 120,
    renderCell: (e: { row: any }) =>  e.row.score? e.row.score: 'N/A' //TODO: replace NA with default value or delete NA after field is added   
  };

  const colComments = {
    field: 'comments',
    headerName: t('comments.label'), // t('status.label')
    width: 120,
    renderCell: (e: { row: any }) =>  e.row.comments? e.row.comments: 'N/A' //TODO: replace NA with default value or delete NA after field is added   
  };

  const colRecommendations = {
    field: 'recommendations',
    headerName: t('recommendations.label'), // t('status.label')
    width: 120,
    renderCell: (e: { row: any }) =>  e.row.recommendations? e.row.recommendations: 'N/A' //TODO: replace NA with default value or delete NA after field is added   
  };

  const colScienceCategory = {
    field: 'scienceCategory',
    headerName: t('scienceCategory.label'), // t('status.label')
    width: 120,
  ...
  const colSrcNet = {
    field: 'srcNet',
    headerName: t('srcNet.label'), // t('status.label')
    width: 120,
    renderCell: (e: { row: any }) =>  e.row.srcNet? e.row.srcNet: 'N/A' //TODO: replace NA with default value or delete NA after field is added   
  };
  ...
  const colDateAssigned = {
    ...
    renderCell: (e: { row: any }) =>{
      console.log('e', e)
      return e.row.dateAssigned? presentDate(e.row.dateAssigned)  : 'N/A' // add this back if you need time + ' ' + presentTime(e.row.dateAssigned)
    }
  };
  ...
  const stdColumns = [
    ...[colId, colTitle, colStatus, colRank, colNewField, colUpdated, colDateAssigned, colActions]
  ];
  ...
  return (
    <>
    <PageBannerPMT title={t('menuOptions.reviews')} backBtn={backButton()} />
      <Grid container p={5} pt={0} direction="row" alignItems="center" justifyContent="space-around">
        {/* <Grid item xs={12}>
          {pageDescription()}
        </Grid> */}
